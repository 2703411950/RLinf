...
class NoAutoResetSyncVectorEnv(SyncVectorEnv):
    """SyncVectorEnv that does not auto-reset sub-envs when an episode ends.

    ``RealWorldEnv`` applies its own reset logic in ``step`` / ``_handle_auto_reset``.
    Gymnasium 1.x renamed internal step buffers to ``_terminations`` / ``_truncations``;
    delegate to :class:`SyncVectorEnv` with ``autoreset_mode=DISABLED`` instead of
    overriding ``step`` with outdated attribute names (``_terminateds`` / ``_truncateds``).
    """
    ...
    def __init__(
        ...
        # - newer: accepts AutoresetMode.DISABLED
        # - older: accepts string value "disabled"
        autoreset_mode = (
